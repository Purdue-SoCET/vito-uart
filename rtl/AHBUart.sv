...
module AHBUart #(
    ...
    logic buffer_clear;
    logic [1:0] set_size_parity;
    logic PARITY_EN, PARITY_VAL;
	...
	always_ff @(posedge clk, negedge nReset) begin
        if(!nReset) begin
            ...
            buffer_clear <= 1'b1;
	          set_size_parity <= 2'b00; /// {OFF, 0 for even parity}
	    ...
	    if(bp.addr == SET_PARITY && bp.wen) begin
		    set_size_parity <= bp.wdata[1:0]; // sets the parity size 
             end //else begin
		  //   set_size_parity <= set_parity; // the parity maintains itself if the address is enabled...
	    // end
        end
    end
    ...
    UartRxEn uartRx (
        ...
        .done(rxDone),
        .err (rxErr),
        .* // set_size_parity is configured here.
    );


    logic [3:0] BIT_COUNT;
    assign BIT_COUNT = 8; // set to be the same for now cuz im lazy..
    UartTxEn uartTx (
        .en   (txClk),
        .data (txData),
        .valid(txValid),
